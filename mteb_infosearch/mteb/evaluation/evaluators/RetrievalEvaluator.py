    ...
    def search_cross_rerank(
        ...
        **kwargs,
        ...
        for result in self.previous_results:
            qid = result['qid']
            query = result['query']
            docs = result['docs']  # a list of document ids or document content
            docs_content = [
                (doc_id, corpus[doc_id].get("title", "") + " " + corpus[doc_id]["text"])
                for doc_id in docs if doc_id in corpus
            ]
            
            # Now each `docs_content` contains a list of (doc_id, doc_content)
            if docs_content:
                pairs.append((query, docs_content, instructions[query] if instructions is not None else None, qid))

        logger.info(f"Reranking the top {top_k} in batches... This might take a while!")
        itr = range(0, len(pairs), 1)

        results = {qid: {} for qid in [p[3] for p in pairs]}  # p[3] is the qid
        for batch_num, corpus_start_idx in enumerate(
            ...
            # Processing each query with its list of documents (docs_in_pair contains multiple docs per query)
            for i, (query, docs, instructions) in enumerate(zip(queries_in_pair, docs_in_pair, instructions_in_pair)):
                doc_ids, doc_contents = zip(*docs)  # Split into doc_ids and doc_contents

                # CrossEncoder can't take instructions, so add them to query if needed
                if isinstance(self.model, CrossEncoder):
                    query = f"{query} {instructions}".strip() if instructions is not None else query
                    scores = self.model.predict(query, doc_contents)
                else:
                    # If the model can handle instructions
                    scores = self.model.predict(query, doc_contents)

                # Assign the scores to the results dictionary
                for doc_id, score in zip(doc_ids, scores):
                    results[query_ids[i]][doc_id] = float(score)
