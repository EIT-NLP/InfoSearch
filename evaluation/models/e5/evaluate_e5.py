...
# intfloat/e5-mistral-7b-instruct, intfloat/e5-small-v2， Alibaba-NLP/gte-Qwen2-7B-instruct, nvidia/NV-Embed-v2

parser = argparse.ArgumentParser(description='evaluation for BEIR benchmark')
parser.add_argument('--model_name_or_path', default='intfloat/e5-large-v2',
                    type=str, metavar='N', help='which model to use')
parser.add_argument('--output_dir', default='e5-large',
                    type=str, metavar='N', help='output directory')
parser.add_argument('--batch_size', default=32, type=int, metavar='N', help='batch size')
...
parser.add_argument('--doc_as_query', action='store_true',
                    help='use query prefix for passages, only used for Quora as it is a symmetric task')
...
assert args.pool_type in ['cls', 'avg', 'last', 'weightedavg', 'attention'], 'pool_type should be cls / avg / last'
...
class RetrievalModel(DRESModel):
    ...
    def encode_corpus(self, corpus: List[Dict[str, str]], reset_cache: bool = False, **kwargs) -> np.ndarray:
    ...
    def _do_encode(self, input_texts: List[str]) -> np.ndarray:
        ...
        for start_idx in tqdm.tqdm(range(0, len(input_texts), batch_size), desc='encoding', mininterval=10):
            ...
            with torch.cuda.amp.autocast():
                ...
                # embeds = pool(outputs['sentence_embeddings'], batch_dict['attention_mask'], args.pool_type) #For Nv Embed
